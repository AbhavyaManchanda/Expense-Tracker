...
import { RiProgressLine } from "react-icons/ri";
...
const RecentTransactions = ({ data }) => {
  return (
    <div className="flex-1 w-full py-20">
      ...
      <div className="mt-5 overflow-x-auto">
        <table className="w-full border-b border-gray-300 dark:border-gray-700">
          <thead className="w-full text-left text-black dark:text-gray-400">
            <tr>
          ...
          <tbody>
            {data?.map((item, index) => (
               <tr
      key={index}
      className="text-sm text-gray-600 border-b border-gray-200 dark:border-gray-700 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-300"
    >
      <td className="py-4">
    ...
    <td className="flex items-center gap-2 px-2 py-3">
      {item.status === "Pending" && (
        <RiProgressLine className="text-amber-600" size={24} />
      )}
